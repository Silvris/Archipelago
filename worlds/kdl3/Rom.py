...
def patch_rom(multiworld: MultiWorld, player: int, patch: KDL3ProcedurePatch, heart_stars_required: int,
    ...
    if multiworld.music_shuffle[player] > 0:
        if multiworld.music_shuffle[player] == 1:
            ...
            # Zero
            patch.write_token(APTokenTypes.WRITE, 0x9AE79, bytes([music_map[0x18]]))
            # Heart Star success and fail
            patch.write_token(APTokenTypes.WRITE, 0x4A388, bytes([music_map[0x08]]))
            patch.write_token(APTokenTypes.WRITE, 0x4A38D, bytes([music_map[0x1D]]))
        elif multiworld.music_shuffle[player] == 2:
            for room in room_music:
                patch.write_token(APTokenTypes.WRITE, room + 2,
                                  bytes([multiworld.per_slot_randoms[player].choice(music_choices)]))
            for i in range(5):
                # level themes
                patch.write_token(APTokenTypes.WRITE, 0x133F2 + i,
                                  bytes([multiworld.per_slot_randoms[player].choice(music_choices)]))
            # Zero
            patch.write_token(APTokenTypes.WRITE, 0x9AE79,
                              bytes([multiworld.per_slot_randoms[player].choice(music_choices)]))
            # Heart Star success and fail
            patch.write_token(APTokenTypes.WRITE, 0x4A388,
                              bytes([multiworld.per_slot_randoms[player].choice(music_choices)]))
            patch.write_token(APTokenTypes.WRITE, 0x4A38D,
                              bytes([multiworld.per_slot_randoms[player].choice(music_choices)]))

    # boss requirements
    patch.write_token(APTokenTypes.WRITE, 0x3D000,
                      struct.pack("HHHHH", boss_requirements[0], boss_requirements[1], boss_requirements[2],
                                  boss_requirements[3], boss_requirements[4]))
    patch.write_token(APTokenTypes.WRITE, 0x3D00A,
                      struct.pack("H", heart_stars_required if multiworld.goal_speed[player] == 1 else 0xFFFF))
    patch.write_token(APTokenTypes.WRITE, 0x3D00C, struct.pack("H", multiworld.goal_speed[player]))
    patch.write_token(APTokenTypes.WRITE, 0x3D010, struct.pack("H", multiworld.death_link[player]))
    patch.write_token(APTokenTypes.WRITE, 0x3D012, struct.pack("H", multiworld.goal[player]))
    patch.write_token(APTokenTypes.WRITE, 0x3D014, struct.pack("H", multiworld.stage_shuffle[player]))
    patch.write_token(APTokenTypes.WRITE, 0x3D016, struct.pack("H", multiworld.ow_boss_requirement[player]))
    patch.write_token(APTokenTypes.WRITE, 0x3D018, struct.pack("H", multiworld.consumables[player]))
    ...
    # write jumping goal
    patch.write_token(APTokenTypes.WRITE, 0x94F8, struct.pack("H", multiworld.jumping_target[player]))
    patch.write_token(APTokenTypes.WRITE, 0x944E, struct.pack("H", multiworld.jumping_target[player]))
    ...
    patch.write_token(APTokenTypes.WRITE, 0x7FD9, bytes([multiworld.game_language[player]]))
    ...
    # test other types of tokens here
    # writes rom name to ROM header, initally written elsewhere
    patch.write_token(APTokenTypes.COPY, 0x7FC0, (21, 0x3C000))
    patch.write_token(APTokenTypes.RLE, 0x3C020, (32, 0xEA))
    patch.write_token(APTokenTypes.AND_8, 0x3C020, 0xF0)  # result should be 0xE0
    patch.write_token(APTokenTypes.OR_8, 0x3C021, 0x15)  # result should be 0xFF
    patch.write_token(APTokenTypes.XOR_8, 0x3C022, 0xEB)  # result should be 0x01
