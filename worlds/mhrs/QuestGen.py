...
stage_set_max = {
    ...
    9: 1,
    10: 1,
    11: 1, #2 valid areas, however there exists no transition between the two tmk (and I'm unsure how to set between the two)
    12: 11,
    13: 14,
    14: 1,
...
monster_choices = [ # Monsters that are always an option for generation
    # Rathian
    # Rathalos
    3, # Khezu
    4, # Basarios
...
def can_afflict(monster: int):
    if monster in {24, 25, 27, 58, 72, 96, 124, 132, 135, 549, 594, 1379, 1366, 2072, 2073, 2075, 2134, 1793, 1794, 1799, 1849, 1852, 1874}:
        return False
    else:
        return True
...
def generate_valid_monster(stage: int, slot_random: Random) -> int:
    monster = slot_random.choice(monster_choices)
    if monster == 135 and stage != 15:
        return generate_valid_monster(stage, slot_random)
    if monster == 1379 and stage != 11:
        return generate_valid_monster(stage, slot_random)
    return monster

def randomize_quest(multiworld: MultiWorld, player: int, quest_id: int, quest_targets=None) -> dict:
    ...
    for i in range(0, 5):
        if i < len(quest_targets):
            ...
            monster = generate_valid_monster(stage, multiworld.per_slot_randoms[player])
    ...
    for i in range(5):
        enemy["Monsters"][i]["SetName"] = f"AP_Custom_{multiworld.per_slot_randoms[player].randint(1,stage_set_max[stage])}"
