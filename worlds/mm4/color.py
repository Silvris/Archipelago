...
if TYPE_CHECKING:
    ...
    from .rom import MM4ProcedurePatch
...
MM4_COLORS: dict[str, tuple[int, int]] = {
    names.flash_stopper: (0x20, 0x14),
    names.rain_flush: (0x20, 0x29),
    names.drill_bomb: (0x10, 0x16),
    names.pharaoh_shot: (0x36, 0x26),
    names.ring_boomerang: (0x38, 0x18),
    names.dust_crusher: (0x20, 0x00),
    names.dive_missile: (0x20, 0x11),
    names.skull_barrier: (0x3C, 0x21),
    names.rush_coil: (0x30, 0x15),
    names.rush_jet: (0x30, 0x15),
    names.rush_marine: (0x30, 0x15),
    names.wire_adaptor: (0x30, 0x15),
    names.balloon_adaptor: (0x30, 0x15),
    names.charge_buster: (0x3A, 0x2A),
    names.bright_man_stage: (0x20, 0x26),
    names.toad_man_stage: (0x38, 0x29),
    names.drill_man_stage: (0x16, 0x10),
    names.pharaoh_man_stage: (0x20, 0x27),
    names.ring_man_stage: (0x28, 0x16),
    names.dust_man_stage: (0x10, 0x21),
    names.dive_man_stage: (0x20, 0x1C),
    names.skull_man_stage: (0x20, 0x10),
    names.cossack_1_stage: (0x27, 0x16),
    names.cossack_2_stage: (0x27, 0x16),
    names.cossack_3_stage: (0x27, 0x16),
    names.cossack_4_stage: (0x27, 0x16),
}

MM4_KNOWN_COLORS: dict[str, tuple[int, int]] = {
    **MM4_COLORS,
    # Metroid series
...
palette_pointers: dict[str, list[int]] = {
    "Mega Buster": [0x79B14, 0x792CA, 0x7C864],
    "Charged Buster": [],
    "Flash Stopper": [0x79B44],
    "Rain Flush": [0x79B24],
    "Drill Bomb": [0x79B38],
    "Pharaoh Shot": [0x79B40],
    "Ring Boomerang": [0x79B34],
    "Dust Crusher": [0x79B3C],
    "Dive Missile": [0x79B30],
    "Skull Barrier": [0x79B48],
    "Rush Coil": [0x79B18],
    "Rush Jet": [0x79B1C],
    "Rush Marine": [0x79B20],
    "Wire Adapter": [0x79B28],
    "Balloon Adapter": [0x79B2C],
    "Bright Man": [0x7362E, 0x53BD4],
    "Toad Man": [0x73636, 0x53BE5],
    "Drill Man": [0x7363E, 0x53BAA],
    "Pharaoh Man": [0x73646, 0x53BBF],
    "Ring Man": [0x7364E, 0x53B6A],
    "Dust Man": [0x73656, 0x53B88],
    "Dive Man": [0x7365E, 0x53B99],
    "Skull Man": [0x73666, 0x53B77],
}


def check_for_known_worlds():
    if "worlds.mm1" in sys.modules:
        try:
            mm1 = sys.modules["worlds.mm1"]
            MM4_KNOWN_COLORS.update(mm1.color.MM1_COLORS)
            for item in MM4_COLORS:
                mm1.color.add_color_to_mm1(item, MM4_COLORS[item])
        except AttributeError:
            pass
    if "worlds.mm2" in sys.modules:
        try:
            mm2 = sys.modules["worlds.mm2"]
            MM4_KNOWN_COLORS.update(mm2.color.MM2_COLORS)
            for item in MM4_COLORS:
                mm2.color.add_color_to_mm2(item, MM4_COLORS[item])
        except AttributeError:
    ...
    if "worlds.mm3" in sys.modules:
        try:
            ...
            MM4_KNOWN_COLORS.update(mm3.color.MM3_COLORS)
            for item in MM4_COLORS:
                mm3.color.add_color_to_mm3(item, MM4_COLORS[item])
        except AttributeError:
            pass


def add_color_to_mm4(name: str, color: tuple[int, int]) -> None:
    """
    ...
    """
    MM4_KNOWN_COLORS[name] = validate_colors(*color)
...
def get_colors_for_item(name: str) -> tuple[tuple[int, int, int], tuple[int, int, int]]:
    if name in MM4_KNOWN_COLORS:
        return expand_colors(*MM4_KNOWN_COLORS[name])
...
def write_palette_shuffle(world: "MM4World", rom: "MM4ProcedurePatch") -> None:
    ...
    if palette_shuffle != 0:
        if palette_shuffle > 1:
            ...
            shuffled_colors = list(MM4_COLORS.values())[:-4]  # don't need cossack colors
    ...
    for character in palettes_to_write:
        for pointer in palette_pointers[character]:
            if character in ("Bright Man", "Toad Man", "Drill Man", "Pharaoh Man",
                             "Ring Man", "Dust Man", "Dive Man", "Skull Man"):
                # these two have two palettes
                colors = palettes_to_write[character]
                pal1 = extrapolate_color(colors[0])
                pal2 = extrapolate_color(colors[1])
        ...
        if character == "Mega Buster":
            ...
            rom.write_byte(0x7929B, palettes_to_write[character][0])
            rom.write_bytes(0x7929D, [palettes_to_write[character][1]] * 13)
            rom.write_byte(0x792AB, palettes_to_write[character][1])
        if character == "Charged Buster":
            rom.write_byte(0x7929A, palettes_to_write[character][0])
            rom.write_byte(0x7929C, palettes_to_write[character][0])
            rom.write_byte(0x792AA, palettes_to_write[character][1])
            rom.write_byte(0x792AC, palettes_to_write[character][1])
