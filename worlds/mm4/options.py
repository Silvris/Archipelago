...
from Options import Choice, Toggle, DeathLink, TextChoice, Range, OptionDict, PerGameCommonOptions, Visibility
...
class StrictWeaknesses(Toggle):
    ...
    Weapons that only do 1-3 damage to bosses no longer deal damage.
    """
...
class IncludeRush(Toggle):
    """
    If enabled alongside Random Weaknesses, Rush Coil and Jet will also have their damage to bosses
    randomized separately from the Mega Buster.
    If enabled alongside Enemy Weaknesses, Rush Coil/Jet/Marine will also have their damage to enemies
    randomized separately from the Mega Buster.
    """
    display_name = "Randomize Rush Damage"


class Wily3Requirement(Range):
    """
    Change the amount of Robot Masters that are required to be defeated for
    the door to the Wily Machine to open.
    """
    display_name = "Wily 3 Requirement"
    default = 8
    range_start = 1
    range_end = 8
